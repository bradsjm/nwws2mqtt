...
class WeatherWireStatsCollector:
    ...
    def record_muc_join_failure(self, reason: str) -> ReceiverStatsEvent:
        """Record a MUC room join failure."""
        labels = {"receiver": self.receiver_id, "reason": reason}
        ...
        self.collector.record_error(
            "muc_join_failure", operation="muc_join", labels=labels
        )
        ...
        return ReceiverStatsEvent(
            receiver_id=self.receiver_id,
            metric_name="muc_join_failures_total",
            metric_value=current_value,
            success=False,
            details={"reason": reason},
        )

    def record_subscription_verification_success(self) -> ReceiverStatsEvent:
        """Record successful subscription verification."""
        labels = {"receiver": self.receiver_id}
        ...
        current_value = (
            self.collector.get_metric_value(
                "subscription_verifications_success_total", labels=labels
            )
            or 0
        )
        ...
        return ReceiverStatsEvent(
            receiver_id=self.receiver_id,
            metric_name="subscription_verifications_success_total",
            metric_value=current_value,
        )

    def record_subscription_verification_failure(self) -> ReceiverStatsEvent:
        """Record subscription verification failure."""
        labels = {"receiver": self.receiver_id}
        ...
        current_value = (
            self.collector.get_metric_value(
                "subscription_verification_failures_total", labels=labels
            )
            or 0
        )
        ...
        return ReceiverStatsEvent(
            receiver_id=self.receiver_id,
            metric_name="subscription_verification_failures_total",
            metric_value=current_value,
            success=False,
        )
    ...
    def record_presence_sent(self) -> ReceiverStatsEvent:
        """Record presence stanza sent."""
        labels = {"receiver": self.receiver_id}
        ...
        current_value = (
            self.collector.get_metric_value("presence_sent_total", labels=labels) or 0
        )

        return ReceiverStatsEvent(
            receiver_id=self.receiver_id,
            metric_name="presence_sent_total",
            metric_value=current_value,
        )

    def record_presence_send_failure(self, error: str) -> ReceiverStatsEvent:
        """Record presence send failure."""
        labels = {"receiver": self.receiver_id, "error": error}
        ...
        current_value = (
            self.collector.get_metric_value(
                "errors_total",
                labels={**labels, "operation": "presence_send"},
            )
            or 0
        )
        ...
        return ReceiverStatsEvent(
            receiver_id=self.receiver_id,
            metric_name="presence_send_failures_total",
            metric_value=current_value,
            success=False,
            details={"error": error},
        )

    def update_muc_joined_status(self, *, is_joined: bool) -> ReceiverStatsEvent:
        """Update the MUC joined status gauge."""
        labels = {"receiver": self.receiver_id}
        status_value = 1.0 if is_joined else 0.0
        ...
        return ReceiverStatsEvent(
            receiver_id=self.receiver_id,
            metric_name="muc_joined_status",
            metric_value=status_value,
        )

    def update_subscription_verified_status(self, *, is_verified: bool) -> ReceiverStatsEvent:
        """Update the subscription verified status gauge."""
        labels = {"receiver": self.receiver_id}
        status_value = 1.0 if is_verified else 0.0
        ...
        return ReceiverStatsEvent(
            receiver_id=self.receiver_id,
            metric_name="subscription_verified_status",
            metric_value=status_value,
        )
