...
def create_counter(name: str, documentation: str, labelnames=None, registry=None):
    ...
    counter = Counter(name, documentation, labelnames=labelnames or [], registry=registry)
    return counter


class PrometheusMetricsExporter:
    """
    PrometheusMetricsExporter is responsible for exporting NWWS2MQTT application statistics as Prometheus metrics.

    This class initializes and manages a Prometheus HTTP server that exposes various application, connection, message processing, and output handler metrics. It periodically updates these metrics based on the current statistics collected from the application.
    ...
    def __init__(self, stats_collector: StatsCollector, port: int = 8080, update_interval: int = 30):
        ...
        self._update_task: Optional[LoopingCall] = None
        ...
        logger.info("Prometheus metrics exporter initialized", port=port, update_interval=update_interval)

    def _init_metrics(self) -> None:
        """Initialize all Prometheus metrics."""

        # Application info
        self.app_info = Info("nwws2mqtt_application_info", "Application information", registry=self.registry)

        # Application uptime
        self.app_uptime_seconds = Gauge(
            "nwws2mqtt_application_uptime_seconds", "Application uptime in seconds", registry=self.registry
        )

        # Connection metrics
        self.connection_status = Gauge(
            "nwws2mqtt_connection_status", "XMPP connection status (1=connected, 0=disconnected)", registry=self.registry
        )

        self.connection_uptime_seconds = Gauge(
            "nwws2mqtt_connection_uptime_seconds", "Current connection uptime in seconds", registry=self.registry
        )

        self.connection_total_connections = create_counter(
            "nwws2mqtt_connection_total_connections", "Total number of connections made", registry=self.registry
        )

        self.connection_total_disconnections = create_counter(
            "nwws2mqtt_connection_total_disconnections", "Total number of disconnections", registry=self.registry
        )

        self.connection_reconnect_attempts = create_counter(
            "nwws2mqtt_connection_reconnect_attempts_total", "Total number of reconnection attempts", registry=self.registry
        )

        self.connection_auth_failures = create_counter(
            "nwws2mqtt_connection_auth_failures_total", "Total number of authentication failures", registry=self.registry
        )

        self.connection_errors = create_counter(
            "nwws2mqtt_connection_errors_total", "Total number of connection errors", registry=self.registry
        )

        self.outstanding_pings = Gauge(
            "nwws2mqtt_connection_outstanding_pings", "Number of outstanding ping requests", registry=self.registry
        )

        # Message processing metrics
        self.messages_received_total = create_counter(
            "nwws2mqtt_messages_received_total", "Total number of messages received", registry=self.registry
        )

        self.messages_processed_total = create_counter(
            "nwws2mqtt_messages_processed_total", "Total number of messages successfully processed", registry=self.registry
        )
        ...
        self.messages_published_total = create_counter(
            "nwws2mqtt_messages_published_total", "Total number of messages published to output handlers", registry=self.registry
        )

        self.message_processing_success_rate = Gauge(
            "nwws2mqtt_message_processing_success_rate", "Message processing success rate as percentage", registry=self.registry
        )
        ...
        self.sources_total = create_counter(
            "nwws2mqtt_sources_total", "Total count by source", labelnames=["source"], registry=self.registry
        )

        # AFOS code metrics
        self.afos_codes_total = create_counter(
            "nwws2mqtt_afos_codes_total", "Total count by AFOS code", labelnames=["afos_code"], registry=self.registry
        )
    ...
    def _update_metrics(self) -> None:
        ...
        try:
            ...
            self._update_output_handler_metrics(stats)
    ...
    def _update_application_metrics(self, stats: ApplicationStats) -> None:
        ...
        self.app_info.info(
            {
                "version": "0.1.0",  # Could be made configurable
                "start_time": stats.start_time.isoformat(),
            }
        )

        self.app_uptime_seconds.set(stats.running_time_seconds)
    ...
    def _update_connection_metrics(self, stats: ApplicationStats) -> None:
        ...
        total_connections_diff = stats.connection.total_connections - self._last_connection_values["total_connections"]
        ...
        total_disconnections_diff = stats.connection.total_disconnections - self._last_connection_values["total_disconnections"]
        if total_disconnections_diff > 0:
            self.connection_total_disconnections.inc(total_disconnections_diff)
            self._last_connection_values["total_disconnections"] = stats.connection.total_disconnections

        reconnect_attempts_diff = stats.connection.reconnect_attempts - self._last_connection_values["reconnect_attempts"]
        ...
        auth_failures_diff = stats.connection.auth_failures - self._last_connection_values["auth_failures"]
        ...
        connection_errors_diff = stats.connection.connection_errors - self._last_connection_values["connection_errors"]
    ...
    def _update_message_metrics(self, stats: ApplicationStats) -> None:
        ...
        if not hasattr(self, "_last_message_values"):
        ...
        # Update message counters with differences
        ...
        processed_diff = stats.messages.total_processed - self._last_message_values["total_processed"]
        if processed_diff > 0:
            self.messages_processed_total.inc(processed_diff)
            self._last_message_values["total_processed"] = stats.messages.total_processed

        published_diff = stats.messages.total_published - self._last_message_values["total_published"]
        if published_diff > 0:
        ...
        for error_type, count in stats.messages.processing_errors.items():
            last_count = self._last_message_values["processing_errors"].get(error_type, 0)
            diff = count - last_count
            if diff > 0:
                self.messages_failed_total.labels(error_type=error_type).inc(diff)
                self._last_message_values["processing_errors"][error_type] = count
        ...
        for source, count in stats.messages.sources.items():
            last_count = self._last_message_values["sources"].get(source, 0)
            diff = count - last_count
            if diff > 0:
                self.sources_total.labels(source=source).inc(diff)
                self._last_message_values["sources"][source] = count

        # Update AFOS code counters
        for afos_code, count in stats.messages.afos_codes.items():
            last_count = self._last_message_values["afos_codes"].get(afos_code, 0)
            diff = count - last_count
            if diff > 0:
                self.afos_codes_total.labels(afos_code=afos_code).inc(diff)
                self._last_message_values["afos_codes"][afos_code] = count

    def _update_output_handler_metrics(self, stats: ApplicationStats) -> None:
        ...
        for handler_name, handler_stats in stats.output_handlers.items():
            labels = {"handler_name": handler_name, "handler_type": handler_stats.handler_type}
            ...
            # Update counters with differences
            published_diff = handler_stats.total_published - self._last_handler_values[handler_name]["total_published"]
            if published_diff > 0:
                self.output_handler_published_total.labels(**labels).inc(published_diff)
                self._last_handler_values[handler_name]["total_published"] = handler_stats.total_published

            failed_diff = handler_stats.total_failed - self._last_handler_values[handler_name]["total_failed"]
            if failed_diff > 0:
            ...
            errors_diff = handler_stats.connection_errors - self._last_handler_values[handler_name]["connection_errors"]
            if errors_diff > 0:
                self.output_handler_connection_errors.labels(**labels).inc(errors_diff)
                self._last_handler_values[handler_name]["connection_errors"] = handler_stats.connection_errors
