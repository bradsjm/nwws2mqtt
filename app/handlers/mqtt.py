...
class MQTTOutputHandler(OutputHandler):
    ...
    async def _start_handler(self) -> None:
        ...
        try:
            ...
            logger.info("Connecting to MQTT broker", handler="mqtt", broker=self.config.mqtt_broker, port=self.config.mqtt_port)
            ...
            try:
                ...
                raise ConnectionError("Timeout connecting to MQTT broker")
    ...
    async def publish(self, source: str, afos: str, product_id: str, structured_data: str, subject: str = "") -> None:
        ...
        if not self.client or not self._connected:
            logger.warning("MQTT client not connected, skipping publish", handler="mqtt")
        ...
        try:
            ...
            result = self.client.publish(
                topic, structured_data, qos=self.config.mqtt_qos, retain=self.config.mqtt_retain, properties=properties
            )
            ...
            if result.rc == mqtt.MQTT_ERR_SUCCESS:
                ...
                logger.debug("Published product to MQTT topic", handler="mqtt", product_id=product_id, topic=topic)
            else:
                ...
                for topic in expired_topics:
                    try:
                        ...
                        if result.rc == mqtt.MQTT_ERR_SUCCESS:
                            ...
                            logger.warning(
                                "Failed to remove expired message from topic", handler="mqtt", topic=topic, return_code=result.rc
                            )
                    except Exception as e:
                        logger.error("Error removing expired message from topic", handler="mqtt", topic=topic, error=str(e))
        ...
        for topic in list(self._published_topics.keys()):
            try:
                ...
                if result.rc == mqtt.MQTT_ERR_SUCCESS:
                    ...
                    logger.warning(
                        "Failed to remove retained message from topic", handler="mqtt", topic=topic, return_code=result.rc
                    )
            except Exception as e:
                logger.error("Error removing retained message from topic", handler="mqtt", topic=topic, error=str(e))
